...
# -------------------------
# Simple Product Catalog (Ramu Kaka Shop)
# -------------------------
...
def find_product_by_ref(ref_text: str, candidates: Optional[List[Dict]] = None) -> Optional[Dict]:
    ...
    Heuristics:
    - Handle ordinals like 'first/second/third/fourth' within a filtered candidate list.
    ...
    # fallback: ordinals over full candidate list
    for word, idx in ordinals.items():
...
async def show_catalog(
    ...
    """
    userdata = ctx.userdata  # currently unused but kept for future personalization
    ...
    # If query mentions phones or tees, infer category
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Ramu Kaka Shop:"]
...
async def add_to_cart(
    ...
    if not prod:
        return (
            "I couldn't resolve which product you meant. "
            "Try using the item id or say 'show catalog' to hear options."
        )
...
async def show_cart(
    ...
    """Read out the items currently in the cart."""
    userdata = ctx.userdata
...
async def last_order(
    ...
    """Read back the most recent order from the JSON history."""
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Ramu Kaka', the friendly shopkeeper and voice assistant for Ramu Kaka Shop.
        ...
        Rules:
            - Use the provided tools to show the catalog, add items to cart, show the cart, place orders,
              show last order, and clear the cart.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (Ramu Kaka Shop) — Ramu Kaka")
